...
pub fn prvAddTaskToReadyList(pxNewTCB: &TCB_t_link) {
    ...
    // print("Adding task to ready list - Priority: ");
    // let c = ['0' as u8 + uxPriority as u8];
    // unsafe {
    //     print(core::str::from_utf8_unchecked(&c));
    // }
    // print(" Name: ");
    // print(&pxNewTCB.read().pcTaskName);
...
pub extern "C" fn vTaskStartScheduler() {
    print("Step 4.1: Initializing ISR stack...");
    X_ISRSTACK_
        .write()
        .resize(unsafe { CONFIG_ISR_STACK_SIZE_WORDS }, 0);
    // Initialize ISR stack values
    for i in 0..unsafe { CONFIG_ISR_STACK_SIZE_WORDS } {
        X_ISRSTACK_.write()[i] = 0xDEADBEEF;  // 初始化栈内容为特定值
    }
    print("Step 4.2: ISR stack initialized");
    
    print("Step 4.3: Setting scheduler running flag...");
    unsafe {
        XSCHEDULERRUNNING = pdTRUE;
    }
    print("Step 4.4: Scheduler running flag set");
    if cfg!(feature = "configSUPPORT_STATIC_ALLOCATION") {
        print("Step 4.5: Creating idle task...");
        let param: Param_link = 0;
        let arch=size_of::<usize>();
        print(&format!("Step 4.5.1: arch size = {}", arch));
        let stack2ptr: StackType_t_link = &*IDLE_STACK as *const [usize; USER_STACK_SIZE]
            as *const usize as usize
            + USER_STACK_SIZE * arch
            - arch;
        print(&format!("Step 4.5.2: stack2ptr = 0x{}", stack2ptr));
        match xTaskCreateStatic(
            ...
            Some(_) => print("Step 4.5.3: Idle task created"),
            None => print("Step 4.5.3: Idle task creation failed"),
        }
    }
    print("Step 4.6: Setting current TCB...");
    set_current_tcb(Some(Arc::downgrade(&IDLE_p)));
    print("Step 4.7: Current TCB set");
    print("Step 4.8: Starting scheduler...");
...
pub fn taskSELECT_HIGHEST_PRIORITY_TASK() {
    ...
    //TODO: 函数规范化
    print("Selecting highest priority task...==================");
    let max_prio = taskSELECT_HIGHEST_PRIORITY();
    // print(&format!("taskSELECT_HIGHEST_PRIORITY: max_prio = {}", max_prio));
    ...
    print("Current highest priority: ");
    let c = ['0' as u8 + max_prio as u8];
    unsafe {
    print(core::str::from_utf8_unchecked(&c));
    }
    ...
    // 打印任务名称
    if let Some(tcb) = owner.upgrade() {
        print("Selected task name: ");
        print(tcb.read().pcTaskName.as_str());
    } else {
...
pub fn taskSELECT_HIGHEST_PRIORITY() -> usize {
    ...
    print("Searching for highest priority task...");
    for i in 1..16 {
        let j = 16 - i;
        if !list_is_empty(&READY_TASK_LISTS[j]) {
            print("Found task at priority: ");
            let c = ['0' as u8 + j as u8];
            unsafe {
                print(core::str::from_utf8_unchecked(&c));
            }
            return j;
    ...
    print("No tasks found, returning priority 0");
    return 0;
}
...
impl tskTaskControlBlock {
    pub fn get_stack_top(&self) -> StackType_t_link {
        self.pxTopOfStack
    }
